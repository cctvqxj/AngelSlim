            ...
            with torch.no_grad():
                for batch in tqdm(dataloader, desc="calibrating...", total=len(dataloader)):
                    ...
                    try:
                        ...
                        # Catcher raises ValueError after capturing layer-0 inputs;
                        # this is the normal GPTQ flow — no outputs expected.
                        pass

                    # PPL computation is monitoring-only; skip when Catcher intercepted
                    if outputs is not None:
                        try:
                            ...
                            mask = attention_mask.view(-1).to(logits.device).float()
                            loss = loss * mask
                            ...
                            print_info(f"[calibration] PPL failed on sample {calibrated_cnt}: {e}")

...
# === AngelSlim ===
# Import collect_fused_moe_internal_stats for MoE expert calibration.
# Locate vllm/tools/vllm_calibrate_utils.py by walking up from this file's
# directory inside the installed vllm package, identical to the original
# AngelSlim integration scheme.
_angelslim_logger = init_logger(__name__)
# Track which missing-attribute warnings have already been emitted so that
# we only log them once per (layer_name, attr) combination instead of on
# every forward call.
_ANGELSLIM_WARNED_ATTRS: set = set()
try:
    ...
    _angelslim_logger.debug("[AngelSlim] Trying to import from tools_path: %s", _tools_path)
    ...
    _angelslim_logger.warning(
        "[AngelSlim] Import of collect_fused_moe_internal_stats failed: %s. "
        "MoE calibration stats collection will be a no-op.",
        _e,
    )
...
def _prepare_expert_assignment(
    ...
    *,
    ...
    naive_block_assignment = (
        ...
        and not (
            (use_int8_w8a16 or use_int4_w4a16) and block_shape is not None and block_shape[1] > 0
        )
    )
...
class TritonExperts(LoRAExpertsMixin, mk.FusedMoEExpertsModular):
    ...
    def apply(
        ...
        # === AngelSlim ===
        # Collect statistics for the gate_up projection input (pre-quantization
        # FP activation entering the first GEMM of the FusedMoE block).
        # NOTE: vLLM v0.20 routes TRITON unquantized MoE through this
        # TritonExperts.apply() method (a modular FusedMoEExpertsModular
        # kernel), not through the legacy fused_experts_impl function.
        # The hooks must therefore live here, on the path actually executed.
        _layer_name = getattr(w1, "_vllm_layer_name", None)
        if _layer_name is None and "_vllm_layer_name" not in _ANGELSLIM_WARNED_ATTRS:
            logger.warning(
                "[AngelSlim] w1 does NOT have _vllm_layer_name attribute; "
                "MoE calibration stats will be skipped for this layer. "
                "(this message is only logged once)"
            )
            _ANGELSLIM_WARNED_ATTRS.add("_vllm_layer_name")
        _moe_activation_stats = getattr(w1, "_moe_activation_stats_of_model", None)
        if (
            _moe_activation_stats is None
            and "_moe_activation_stats_of_model" not in _ANGELSLIM_WARNED_ATTRS
        ):
            logger.warning(
                "[AngelSlim] w1 does NOT have _moe_activation_stats_of_model "
                "attribute; MoE calibration stats will be skipped for this "
...
class TritonWNA16Experts(TritonExperts):
    ...
    def _supports_current_device() -> bool:
        raise NotImplementedError(
            "TritonWNA16Experts is not yet used by an Oracle. " "This method should not be called."
        )
